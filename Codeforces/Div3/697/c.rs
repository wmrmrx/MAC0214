...
impl Solver {
    fn solve<R: Read, W: Write>(&mut self, sc: &mut In<R>, bf: &mut Out<W>) {
        let (a,b,k) = (sc.read::<u64>(), sc.read::<u64>(), sc.read::<u64>());
        let mut va = sc.read_vec::<u64>(k);
        let mut vb = sc.read_vec::<u64>(k);
        ...
        let mut ca = vec![0 as u64; a];
        let mut cb = vec![0 as u64; b];
